  ...
  private Route createRoute(String routeId, Map<String, Stop> stops, TimeTable tt, IDataService service) {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      for (int j = 0; j < stoptimes.size(); j++) {
        ...
        PublicTransportationTrip t = new PublicTransportationTrip(info.getTripId(), serviceId.startDate(), serviceId.endDate(), exceptions, tripStops,
            tripTimes, traces);
        toAdd.addLast(t);

        if (!tripInfo.containsKey(t.getTripId()))
          tripInfo.put(t.getTripId(), t);
      }
      trips.add(toAdd);
    }
