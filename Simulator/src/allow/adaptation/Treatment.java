...
public class Treatment extends Loggable {
	...
	// number
	// of Ii issues
	// private int[] ivIssuesMobility = { 0, 0, 0, 0, 0 }; // array containing
	// the
	// number
	private int[] ivIssuesMobility = { 0 };

	private int[] ivIssues = { 0, 0, 0, 0, 0 };

	private String[] ivIssueNamesDrones = { "Drone1Fault", "Drone3Fault",
			"IntruderDetected", "ObstacleFound", "CameraFault" }; // array

	// private String[] ivIssueNamesMobility = { "IntenseTraffic",
	// "RouteBlocked", "PassengerDelay", "DriversStrike",
	// "CPAPassengerDelay" }; // array

	private String[] ivIssueNamesMobility = { "BUS_BREAKDOWN" }; // array
	...
	public Treatment(int id, int v1Value, String scenario) {
		...
		if (v1Value == 1) {
			if (this.scenario.equalsIgnoreCase("Mobility")) {
				this.ivIssuesMobility[random.nextInt(1)] = 1;
		...
		List<Integer> randomValues = Utilities.generateRandomValues(
				(int) ((100) * ((double) v1Value / 100)), 5);
		int currentIndexRandomValues = 0;
		if (scenario.equalsIgnoreCase("Mobility")) {
			for (int i = 0; i < this.ivIssuesMobility.length; i++) {
				this.ivIssuesMobility[i] = randomValues
						.get(currentIndexRandomValues);
				currentIndexRandomValues++;
			}
		} else {
			for (int i = 0; i < this.ivIssuesDrones.length; i++) {
				this.ivIssuesDrones[i] = randomValues
						.get(currentIndexRandomValues);
				currentIndexRandomValues++;
			}
		}
	...
	public Treatment(int id, int v1Value, int issueIndex, int othersValue,
		...
		if (othersValue != 100) {
			List<Integer> randomValues = Utilities.generateRandomValues(
					(int) ((100 - othersValue) * ((double) v1Value / 100)), 4);
			int currentIndexRandomValues = 0;
	...
	public String toString() {
		...
		if (this.scenario.equalsIgnoreCase("Drones")) {
			return "Treatment [id=" + this.id + ", iv1=" + iv1
					+ ", ivIssuesDrones=" + Arrays.toString(ivIssuesDrones)
					+ "]";
	...
	public String getCsvFileHeader(String commaDelimiter) {
		String result = "id" + commaDelimiter + "iv1" + commaDelimiter + "iv2"
				+ commaDelimiter + "iv3" + commaDelimiter + "iv4"
				+ commaDelimiter + "iv5" + commaDelimiter + "iv6";
		return result;
	}
	...
	public String toCsv(String commaDelimiter) {
		...
		if (this.scenario.equalsIgnoreCase("Drones")) {
			result += Arrays.toString(ivIssuesDrones).replace(" ", "")
					.replace("[", "").replace("]", "")
					.replace(",", commaDelimiter);
		} else {
			result += Arrays.toString(ivIssuesMobility).replace(" ", "")
					.replace("[", "").replace("]", "")
					.replace(",", commaDelimiter);
		}
