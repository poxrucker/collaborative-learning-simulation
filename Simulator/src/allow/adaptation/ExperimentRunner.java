...
public class ExperimentRunner {
	...
	public static ExperimentRunner getInstance() {
		if (ExperimentRunner.instance == null) {
			...
			ExperimentRunner.memoryBaseline = ExperimentRunner.memoryBean
	...
	public ExperimentResult run(
			...
			for (int i = 0; i < ensembles.size(); i++) {
				for (int j = 0; j < ensembles.get(i).getRolesManagers().size(); j++) {
					...
					if (currentManager.getRole().getType()
						...
						for (int k = 0; k < ensembles.get(i).getRolesManagers()
								.size(); k++) {
							RoleManager rm = ensembles.get(i)
									.getRolesManagers().get(k);
							Role r1 = rm.getRole();
						...
						solution = demo.executeCap(cap, new DummyExecution(),
								scenario, GlobalResult, cat, graph, startEdge,
								ensembles);

						Map.Entry<CATree, Integer> entry = solution.entrySet()
								.iterator().next();
						...
						// Depth
						List path = DijkstraShortestPath.findPathBetween(graph,
								"INIT", lastNode);
						path = DijkstraShortestPath.findPathBetween(graph,
								"INIT", lastNode);
						int depth = path.size();
						...
						while (iterator3.hasNext()) {
							...
							if (tokens.length > 1) {
								...
								if (!rolesName.contains(roleName)
										&& !tokens[0].equalsIgnoreCase("COM")
										&& !tokens[0].equalsIgnoreCase("OR")
										&& !tokens[0].equalsIgnoreCase("AND")) {
		...
		// ExperimentRunner.runtime.freeMemory() -
		// ExperimentRunner.memoryBaseline;
		long usedMemory = ExperimentRunner.memoryBean.getHeapMemoryUsage()
				.getUsed() - ExperimentRunner.memoryBaseline;
		...
		result.setDv6(dv6);
		System.out
				.println("Average number of ensembles used to solve the issue: "
						+ dv6);
		...
		result.setDv7(dv7);
		System.out
				.println("Average number of roles involved in an issue resolution: "
						+ dv7);
