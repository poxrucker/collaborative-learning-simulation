...
import allow.adaptation.model.IssueCommunication.CommunicationStatus;
import allow.adaptation.model.IssueResolution;
import allow.adaptation.model.Target;
import allow.adaptation.model.Target.TargetStatus;
import allow.adaptation.presentation.CANode;
		...
		for (int j = 0; j < communications.size(); j++) {
			...
			if (communications.get(j).getTargets().size() > 1) {
				...
				if (constraint.equalsIgnoreCase("Forall")) {
					Object v3 = cat.insertNode(cat.getDefaultParent(), null,
							"AND", STYLE_AND);
					cat.insertEdge(cat.getDefaultParent(), "", "", v5, v3,
							STYLE_DOTTED_EDGE);
					currentNode = v3;
				} else {
					Object v3 = cat.insertNode(cat.getDefaultParent(), null,
